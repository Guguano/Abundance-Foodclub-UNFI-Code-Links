...
function designModeEditButtonSetup(editButton) {
...
function complexEditButtonSetup(editButton, resetButton) {
    const innerInvoiceTable = document.getElementById('invoiceUserSummaryTotalsTable');

    const totalsCells = innerInvoiceTable.querySelector('tbody').querySelectorAll('td.TotalCol');

    // Store original values
    for (const cell of totalsCells) {
        cell.setAttribute("data-JBPOriginalValue", cell.textContent);
    }
    ...
    resetButton.addEventListener("click", () => {
        for (const cell of totalsCells) {
            cell.textContent = cell.getAttribute("data-JBPOriginalValue");
        }
    });

    for (const cell of totalsCells) {
        // create row reset button
        const rowButton = document.createElement('button');
        rowButton.textContent = "Reset";
        rowButton.addEventListener('click', () => {
            cell.textContent = cell.getAttribute("data-JBPOriginalValue");
        });

        // Add row reset button to row
        const row = cell.parentElement;
        row.appendChild(rowButton);
    }
}

function tryAddInvoiceEditButton() {
    const invoiceTable = document.getElementById('invoiceUserSummaryTable');
    if (invoiceTable) {
        const titleRow = invoiceTable.querySelector('tr');
        const titleTd = titleRow.querySelector('td');

        // create edit button
        const editLabel = document.createElement('label');
        editLabel.textContent = "Editable";
        editLabel.setAttribute("for", "JBPInvoiceEditButton");
        const editButton = document.createElement('input');
        editButton.type = 'checkbox';
        editButton.id = 'JBPInvoiceEditButton';

        // Add edit button to title row
        titleTd.appendChild(editButton);
        titleTd.appendChild(editLabel);

        // create reset button
        const resetButton = document.createElement('button');
        resetButton.textContent = "Clear Changes";

        // Add reset button to title row
        titleTd.appendChild(resetButton);

        // Add button functionality
        complexEditButtonSetup(editButton, resetButton)
    }
}
